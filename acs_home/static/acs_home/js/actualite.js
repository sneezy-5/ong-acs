...
form.addEventListener('submit', function (e) {
    ...
    var iptz=document.querySelectorAll('.iptz')
...
/*systeme de spoiler*/
var lienactus=document.querySelectorAll('.liensactu')

lienactus.forEach(lienactu=>{

    lienactu.addEventListener('click',function(){

        var parent=lienactu.parentNode.parentNode

        /**recup jours mois annee */
        var moisactu=parent.querySelector('.mois')
        var joursactu=parent.querySelector('.jours')
        var anneeactu=parent.querySelector('.annee')

        var valuejours=joursactu.innerHTML
        var valuemois=moisactu.innerHTML
        var valueannee=anneeactu.innerHTML

        /**recup img */
        var img=parent.querySelector('.imgactualite');
        var sourceImgactu=img.src

        /**recup titre */
        var titreactu=parent.querySelector('.titreactu')
        var valeurtitreactu=titreactu.innerHTML

        /**recup text*/
        var text=parent.querySelector('.textactualite')
        var valeurtextactu=text.innerHTML        

        /**transfert vers spoileractualité**/
        
        localStorage.setItem('valeurjours',valuejours)
        localStorage.setItem('valeurmois',valuemois)
        localStorage.setItem('valeurannee',valueannee)
        localStorage.setItem('imgactu',sourceImgactu)
        localStorage.setItem('valeurtitre',valeurtitreactu)
        localStorage.setItem('valeurtext',valeurtextactu)
    })
})